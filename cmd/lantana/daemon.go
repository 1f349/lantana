...
func (d *daemonCmd) Execute(_ context.Context, _ *flag.FlagSet, _ ...interface{}) subcommands.ExitStatus {
	...
	db, err := conf.CreateDBConnection(cnf.Database.ConnectionString, cnf.Database.SchemaPath, hasVal(os.Getenv("DB_UPGRADE"), trues), hasVal(os.Getenv("DB_RESET"), trues))
	if err != nil {
	...
	dbTokens, err := conf.CreateDBConnection(cnf.API.JWT.Database.ConnectionString, cnf.API.JWT.Database.SchemaPath, hasVal(os.Getenv("DB_TOKEN_UPGRADE"), trues), hasVal(os.Getenv("DB_TOKEN_RESET"), trues))
	if err != nil {
